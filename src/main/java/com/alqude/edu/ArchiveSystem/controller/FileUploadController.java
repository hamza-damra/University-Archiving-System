...
public class FileUploadController {
     ...
     * 
     * <p>This endpoint accepts multiple files and uploads them to the specified folder.
     * The current authenticated user must have permission to upload to the target folder.</p>
     * 
     * <h3>Request Parameters:</h3>
     * <ul>
     *   <li><b>files[]</b> - Array of files to upload (required, multipart/form-data)</li>
     ...
     * <ul>
     *   <li><b>400 Bad Request</b> - File validation failed (invalid type, too large, etc.)</li>
    ...
    public ResponseEntity<ApiResponse<List<UploadedFileDTO>>> uploadFiles(
        ...
        try {
            ...
            log.debug("Uploading files for user ID: {}, role: {}", currentUser.getId(), currentUser.getRole());
            ...
            return ResponseEntity.ok(
                    ApiResponse.success(
                            String.format("Successfully uploaded %d file(s)", fileDTOs.size()),
                            fileDTOs
                    )
            );
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("An unexpected error occurred while uploading files. Please try again."));
        }
    }
     ...
     * 
     * <p>This endpoint retrieves all files uploaded to the specified folder.
     * The current authenticated user must have permission to view the folder.</p>
     * 
     * <h3>Request Parameters:</h3>
     * <ul>
     *   <li><b>folderId</b> - Target folder ID (required)</li>
    ...
    public ResponseEntity<ApiResponse<List<UploadedFileDTO>>> getFilesByFolder(
        ...
        try {
            ...
            if (!canViewFolder(folder, currentUser)) {
                ...
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(ApiResponse.error("You do not have permission to view files in this folder"));
            }
            
            // Get files from repository
            List<UploadedFile> files = uploadedFileRepository.findByFolderId(folderId);
            ...
            return ResponseEntity.ok(
                    ApiResponse.success(
                            String.format("Retrieved %d file(s) from folder", fileDTOs.size()),
                            fileDTOs
                    )
            );
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("An unexpected error occurred while retrieving files. Please try again."));
        }
    }
     ...
     * - PROFESSOR role: Can only view their own folders (folder.owner.id == user.id)
