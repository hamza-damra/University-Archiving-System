    ...
    public void run(String... args) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            if (!tableExists(conn, "flyway_schema_history")) {
                System.out.println("ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Fresh database detected (no flyway_schema_history table).");
            ...
            try (Statement stmt = conn.createStatement()) {
                ...
                if (deleted > 0) {
                    System.out.println("ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Removed " + deleted + " failed migration record(s) for version 2");
                    System.out.println("\n=== NEXT STEPS ===");
                    System.out.println("1. Stop the application");
                    System.out.println("2. In application.properties, change:");
                    System.out.println("   spring.flyway.enabled=false  ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢  spring.flyway.enabled=true");
                    ...
                    System.out.println("ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ No failed migrations to clean up.\n");
                }
            }
            
        } catch (Exception e) {
            // Log error but don't throw - allow application to continue
            System.err.println("ÃƒÂ¢Ã…Â¡Ã‚Â  Warning during Flyway cleanup check: " + e.getMessage());
